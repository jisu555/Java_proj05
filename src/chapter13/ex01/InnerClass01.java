...
class A {	//Outer class : 필드, 메소드에 접근 제어자가 붙어있고, 
				//Inner class 를 생각하면 Outer class의 접근 제어자상관 없이 접근
	...
	private int d = 6;	   //다른패키지, 같은패키지에서도 접근불가능, 같은 클래스에서만 접근
	
	void abc() {
		System.out.println("A클래스의 메소드 abc() 입니다.");
	}
	
	//이너 클래스 : 인스턴스 이너클래스, 1. 아웃터클래스의 접근제어자와 상관없이 접근, 
			//2. 기능 구현을 외부의 객체 생성 없이 내부에서 바로 처리
	//class B 내부에 outer class의 필드와 메소드를 활용해서 기능 구현을 함.
	class B {
		void bcd() {
			//1. Outer Class의 필드 접근 : 접근 제어자와 상관없이 모든 필드 접근
