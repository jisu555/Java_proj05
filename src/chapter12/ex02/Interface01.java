...
//추상 클래스 : 추상 메소드를 포함한 클래스, 추상 클래스내의 추상 메소드는 abstract키를 생략하면 안 됨.
abstract class C {
	...
	//추상 클래스 내의 추상 메소드는 abstract키를 붙여야 한다.
	abstract void abc();
...
public class Interface01 {
		...
		/*
		   인터페이스 (Interface) : 내부와 외부를 연결시켜주는 접점, 콘센트
		     ...
		     - 모든 필드는 public static final 이 생략됨
		     	: 값을 수정 할 수 없다.
		     - 모든 메소드는 public abstract 이 생략됨 (예외 : default, static 메소드 : 1.8버전)
	...
	//	C c1 = new C();		//C : 추상클래스
		...
		//2. 추상 클래스 : 구현한 클래스를 객체화해서 타입 지정은 가능
		C c2 = new Cc();
