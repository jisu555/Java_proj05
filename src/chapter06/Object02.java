...
class A {
	...
	// 생성자 : 필드의 내용을 초기화 할 때 사용 (일반적으로), 객체를 생성할 때 생성자를 반드시 호출해야 한다.
	// 리턴 타입이 존재하지 않는다.
	// 메소드 이름이 클래스 이름과 동일한 메소드
	
	//기본 생성자 : 생략될 수 있다. 생성자도 하나의 메소드
	A(){
		//기본 생성자 : 인풋 매개변수가 비어있는 생성자를 기본생성자
	}
	
	// 메소드 : 함수 , 호출 시 작동됨.
	// 리턴타입 메소드명(인풋매개변수) {메소드 호출 시 프로그램 실행 코드}
...
public class Object02 {
	...
	public static void main(String[] args) {
		...
		//자료형
		//클래스명 참조변수 = new A();
		// A : 클래스, 객체자료형, 동일한 패키지 내에 클래스가 존재할 때 import를 없이 사용 가능
		...
		//A클래스를 기준으로 2번째 객체 생성 
			...
			System.out.println(ac.m+ac.n);
			System.out.println(ac.d/ac.m);
