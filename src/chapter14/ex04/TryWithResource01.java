...
public class TryWithResource01 {
	...
	public static void main(String[] args) {
		// Try 구문에서 리소스 자동해제, 객체를 사용 후에 반드시 그 객체를 제거(close())해서 메모리를 최적화
		...
		try {
			...
			//
			System.out.println("IOException이 발생 되었습니다. 하나의 문자만 넣어주세요");
			e.printStackTrace();	 //
		} finally {
			//일반적으로 객체를 사용 후 finally 블락에서 객체를 제거함.
				//객체를 제거 할 때도 예외 처리가 되어야 됨.
			if(ir1 != null) {	//객체가 null일때는 제거하지 않고 객체가 null 이 아닐 때 제거함.
				try {
