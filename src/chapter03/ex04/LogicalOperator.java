...
public class LogicalOperator {
	...
	public static void main(String[] args) {
		...
		//3. 쇼트 서킷 : 논리연산자에서 앞의 내용으로 뒤의 값의 유무없이 식별 가능하면 뒤의 값은 처리하지 않음.
		
		System.out.println(true && true); //쇼트 서킷이 발생되지 않는다. 뒤의 값을 반드시 확인
		System.out.println(false && true); // 쇼트 서킷이 발생
			//AND 연산자에서는 처음 값이 false 가 오게되면 뒤의 값이 어떤값이 오더라도 false
		
		System.out.println(false || true); // 쇼트 서킷이 발생되지않음
		...
		int value2 = 3;
		System.out.println(false & ++value2 > 6); //쇼트 서킷이 발생되지않음 (비트연산자 : &)
		...
		int value4 = 3;
		System.out.println(true | ++value4 > 6); //쇼트 서킷이 발생되지않음 (비트연산자 : |)
		System.out.println(value4);   //4
		
		// XOR (^) : 논리연산, 비트연산 <== 뒤의 값을 반드시 확인해야 결과를 알 수 있으므로 쇼트서킷이 발생되지않음
